...
/** Persisted lifecycle choice, written by the settings section. */
const SETTINGS_PATH = join(LAUNCHER_DIR, 'settings.json')
/** JSON API the settings section reads and writes. */
const SETTINGS_ROUTE = '/dsh-launcher/settings'
/** Where the browser half reports that it loaded, and any failure it hit. */
const CLIENT_REPORT_ROUTE = '/dsh-launcher/client-report'
/** Diagnostic snapshot: the composed client graph plus that report. */
const DIAGNOSTICS_ROUTE = '/dsh-launcher/diagnostics'
/** "Immediately" still floors here: a reload drops and re-opens the page's
 * event stream, so a literal zero would kill the host on every refresh. */
const IMMEDIATE_GRACE_MS = 1000
const SHUTDOWN_MODES = ['immediate', 'never', 'custom']
const MAX_CUSTOM_SECONDS = 86400

function clampSeconds(value, fallback) {
  const seconds = Number(value)
  if (!Number.isFinite(seconds) || seconds < 1) return fallback
  return Math.min(MAX_CUSTOM_SECONDS, Math.round(seconds))
}

/** Read the persisted choice; null when absent or malformed. */
function readSettings() {
  try {
    const raw = JSON.parse(readFileSync(SETTINGS_PATH, 'utf8'))
    const shutdown = raw === null || raw === undefined ? undefined : raw.shutdown
    if (typeof shutdown !== 'string' || !SHUTDOWN_MODES.includes(shutdown)) return null
    return { shutdown, customSeconds: clampSeconds(raw.customSeconds, 25) }
  } catch {
    return null
  }
}
...
function writeSettings(next) {
  try {
    ...
    /* read-only home: the in-memory choice still applies for this run */
  }
}
...
/**
 * Summarize the composed client-plugin graph.
 *
 * This exists so "the settings row never appeared" is answerable from a
 * terminal rather than guessed at: if this package is in `entries`, the graph
 * knows about it and the failure is in the bundle or its slot registration; if
 * it is absent, the `dsh.client` declaration never reached the scan.
 * @param webCtx - Context carrying the webServer service.
 */
function bootGraphSummary(webCtx) {
  try {
    const rows = webCtx.webServer.collectIndexInjections()
    const summary = rows.map((row) => ({ kind: row.kind, name: row.name ?? null }))
    let entries = null
    for (const row of rows) {
      if (row.kind !== 'global') continue
      const value = row.value
      const graph = value !== null && typeof value === 'object' && value.graph !== undefined ? value.graph : value
      if (graph !== null && typeof graph === 'object' && Array.isArray(graph.entries)) {
        entries = graph.entries.map((entry) =>
          entry !== null && typeof entry === 'object' ? (entry.id ?? null) : entry
        )
      }
    }
    return { rows: summary, entries }
  } catch (error) {
...
/**
 * Shut the host down once the last browser page has gone, and expose the
 * choice to the GUI.
 *
 ...
 * @param cfg - Merged plugin config; supplies the initial choice only, since a
 *   saved choice (and any later GUI edit) wins.
 */
function installShutdownControl(ctx, cfg) {
  const persisted = readSettings()
  const state = persisted ?? {
    shutdown: cfg.exitOnBrowserClose === true ? 'custom' : 'never',
    customSeconds: clampSeconds(cfg.exitGraceSeconds, 25)
  }
  ...
  let armed = null
  let leaving = false
  /** Last report from the browser half, so a silent client failure is visible. */
  const clientReport = { loaded: false, error: null, section: null, at: null }

  const enabled = () => state.shutdown !== 'never'
  const graceMs = () =>
    state.shutdown === 'immediate' ? IMMEDIATE_GRACE_MS : Math.max(1, state.customSeconds) * 1000
  ...
  const arm = () => {
    if (leaving || !enabled() || !seenPage || clients.size > 0 || armed !== null) return
  ...
  const publicState = () => ({
    ...
    immediateSeconds: IMMEDIATE_GRACE_MS / 1000,
    effectiveSeconds: graceMs() / 1000,
    pages: clients.size,
    sawPage: seenPage
  })
  ...
  const applySettings = (body) => {
    ...
    const { shutdown } = body
    if (typeof shutdown !== 'string' || !SHUTDOWN_MODES.includes(shutdown)) return false

    let seconds = state.customSeconds
    if (body.customSeconds !== undefined) {
      const requested = Number(body.customSeconds)
      if (!Number.isFinite(requested) || requested < 1 || requested > MAX_CUSTOM_SECONDS) return false
      seconds = Math.round(requested)
    }

    state.shutdown = shutdown
    state.customSeconds = seconds
    writeSettings({ shutdown, customSeconds: seconds })
    say(ctx, `shutdown mode set to ${shutdown}${shutdown === 'custom' ? ` (${seconds}s)` : ''}`)
    sync()
    return true
  }
  ...
  ctx.inject(['webServer'], (webCtx) => {
    ...
    webCtx.webServer.register({
      ...
      handler(req, res) {
        sendJson(res, 200, {
          ok: true,
          plugin: { version: pluginVersion(), mode: state.shutdown, customSeconds: state.customSeconds },
          graph: bootGraphSummary(webCtx),
          client: clientReport
        })
      }
    })
    ...
    webCtx.webServer.register({
      ...
      async handler(req, res) {
        ...
        if (body === null || typeof body !== 'object') {
          sendJson(res, 400, { ok: false, error: 'invalid report' })
          return
        }
        clientReport.loaded = body.loaded === true
        clientReport.error = typeof body.error === 'string' ? body.error.slice(0, 500) : null
        clientReport.section = typeof body.section === 'string' ? body.section.slice(0, 200) : null
        clientReport.at = new Date().toISOString()
        say(ctx, `browser half: loaded=${clientReport.loaded}${clientReport.error === null ? '' : ` error=${clientReport.error}`}`)
    ...
    say(ctx, `shutdown control ready (mode ${state.shutdown}, grace ${graceMs() / 1000}s)`)
  })
}

/**
 * Register launcher maintenance and the host-lifecycle control.
...
export function apply(ctx, config) {
  ...
  try {
    installShutdownControl(ctx, cfg)
  } catch (error) {
    warn(ctx, `could not install the shutdown control: ${reasonOf(error)}`)
  }
}
